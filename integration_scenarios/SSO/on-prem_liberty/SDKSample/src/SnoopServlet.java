...
public class SnoopServlet extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		out.println("<h2>Token Information:</h2>");
		out.println("<TABLE Border=\"2\" WIDTH=\"65%\" BGCOLOR=\"#DDDDFF\">");
		print(out, "Access token", tokens.get("access_token"));
		print(out, "ID token", tokens.get("id_token"));
		out.println("</table><BR><BR>");
		...
		if (e.hasMoreElements()) {
			...
			while (e.hasMoreElements()) {
				...
				out.println(
						"<tr><td>" + escapeChar(name) + "</td><td>" + escapeChar(req.getHeader(name)) + "</td></tr>");
			}
			out.println("</table><BR><BR>");
		}
		...
		if (e.hasMoreElements()) {
			...
			while (e.hasMoreElements()) {
				String name = (String) e.nextElement();
				out.println("<tr><td>" + escapeChar(name) + "</td><td>" + escapeChar(req.getParameter(name))
						+ "</td></tr>");
			}
			out.println("</table><BR><BR>");
		}
		...
		if (e.hasMoreElements()) {
			...
			while (e.hasMoreElements()) {
				String name = (String) e.nextElement();
				out.println("<tr><td>" + escapeChar(name) + "</td><td>" + escapeChar(req.getAttribute(name).toString())
						+ "</td></tr>");
			}
			out.println("</table><BR><BR>");
		}
	...
	private Map<String, String> getTokens() throws IOException {
		...
		// we were authenticated using OIDC, and we want to obtain the access_token and
		// id_token that's part of this session.
		Subject s;
		try {
		...
		for (Hashtable item : privateHashtableCreds) {
			if (item.containsKey("access_token")) {			
				String access_token = item.get("access_token").toString();
				String id_token = item.get("id_token").toString();
